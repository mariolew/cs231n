...
def random_crops(X, crop_shape):
  ...
  #############################################################################
  h = np.random.randint(H - HH, size=N)
  w = np.random.randint(W - WW, size=N)
  # print N, h
  for idx in xrange(N):
      out[idx, :, :, :] = X[idx, :, h[idx]:h[idx] + HH, w[idx]:w[idx] + WW]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  #############################################################################
  multiplier = np.random.rand(N) * (high - low) + low
  out = X * multiplier[:, None, None, None]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
...
def random_tint(X, scale=(-10, 10)):
  ...
  #############################################################################
  color_added = np.random.rand(N, C) * (high - low) + low
  out = X + color_added[:, :, None, None]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
