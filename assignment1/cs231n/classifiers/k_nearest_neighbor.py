...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    # (X -Y)^2 = X.T*X - 2XY + Y.T*Y
    inner_prod = np.dot(X,self.X_train.T)
    X_t = np.square(X).sum(axis=1)
    X_tr = np.square(self.X_train).sum(axis=1)
    dists = np.sqrt(- 2*inner_prod + X_t.reshape(-1, 1) + X_tr)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      # label.                                                                #
      #########################################################################
      label_vec = self.y_train[ind]
      label, counts = np.unique(label_vec, return_counts=True)
